...
def evaluate_epoch(model: torch.nn.Module,
    ...
    for batch in loader:
        ...
        live = [(m, x) for m, x in feats.items() if x is not None]
        if not live:                # весь пример пустой → пропускаем
            continue
        mod_key, x = live[0]        # первая доступная модальность
        out = model({mod_key: x}, modality=mod_key)
